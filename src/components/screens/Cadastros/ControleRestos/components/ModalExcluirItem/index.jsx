...
import { deletarControleRestos } from "services/controleRestos.service";
...
export default ({ closeModal, showModal, item, changePage }) => {
  ...
  const onSubmit = async () => {
    setCarregando(true);
    await deletarControleRestos(item.uuid)
      .then(() => {
        toastSuccess("Controle de restos excluído com sucesso");
      })
      .catch((error) => {
  ...
  return (
    <Modal dialogClassName="modal-50w" show={showModal} onHide={closeModal}>
      <Modal.Header closeButton>
        <Modal.Title>Excluir Controle de Restos</Modal.Title>
      </Modal.Header>
