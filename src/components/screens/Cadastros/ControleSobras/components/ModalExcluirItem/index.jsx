...
import { deletarControleSobras } from "services/controleSobras.service";
...
export default ({ closeModal, showModal, item, changePage }) => {
  ...
  const onSubmit = async () => {
    setCarregando(true);
    await deletarControleSobras(item.uuid)
      .then(() => {
        toastSuccess("Controle de sobras excluído com sucesso");
      })
      .catch((error) => {
  ...
  return (
    <Modal dialogClassName="modal-50w" show={showModal} onHide={closeModal}>
      <Modal.Header closeButton>
        <Modal.Title>Excluir Controle de Sobras</Modal.Title>
      </Modal.Header>
