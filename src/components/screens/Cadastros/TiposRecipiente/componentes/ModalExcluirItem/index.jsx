...
import { deletarTipoRecipiente } from "services/tipoRecipiente.service";
...
export default ({ closeModal, showModal, item, changePage }) => {
  ...
  const onSubmit = async () => {
    setCarregando(true);
    await deletarTipoRecipiente(item.uuid)
      .then(() => {
        toastSuccess("Tipo de recipiente excluído com sucesso");
      })
      .catch((error) => {
  ...
  return (
    <Modal dialogClassName="modal-50w" show={showModal} onHide={closeModal}>
      <Modal.Header closeButton>
        <Modal.Title>Excluir Tipo de Recipiente</Modal.Title>
      </Modal.Header>
